...
layer_cnn0 = tf.layers.Conv2D(64, 2, activation=tf.nn.relu)
layer_cnn1 = tf.layers.Conv2D(64, 1, activation=tf.nn.relu)
...
layer_fc0 = tf.layers.Dense(units=1024, activation=tf.nn.relu)
...
for i in range(iters):
    ...
    optimizer.minimize(loss = lambda: loss(X,y,True), global_step = tf.train.get_global_step())
