...
for i in range(H1, H2):
    for j in range(W1, W2):
        ...
        if r < 0:
            ...
            r = ((r + 0.055) / 1.055) ** 2.4
        mtx_window = np.array([[0.412453, 0.35758, 0.180423], [0.212671, 0.71516, 0.072169], [0.019334, 0.119193, 0.950227]])
        RGB_window = np.array([[r], [g], [b]])
        XYZ_window = np.matmul(mtx_window, RGB_window)

        t_window = XYZ_window[1, 0] / 1.0
        if t_window > 0.008856:
            L_window = 116 * (t_window ** (1 / 3)) - 16
        else:
            L_window = 903.3 * t_window

        mtxL_window.append(L_window)
L_window_max = max(mtxL_window)
L_window_min = min(mtxL_window)

print("maxL_window=",max(mtxL_window),"; minL_window=",min(mtxL_window))
cv2.imshow('tmp', tmp)
...
tmp_2 = np.zeros([rows, cols, bands], dtype=np.float64)
...
for i in range(0, rows) :
    for j in range(0, cols) :
        ...
        if L < L_window_min:
            ...
            L = (L - L_window_min)*100/(L_window_max - L_window_min)
            interval_L = int(round(L))
            h[interval_L] = h[interval_L] + 1
...
for i in range(1,101):
    f[i] = h[i] + f[i-1]
floor[0] = round((f[0]/2) * (101/sum(h))-0.5)
for i in range(1, 101):
    floor[i] = round(((f[i-1]+f[i])/2) * (101/sum(h))-0.5)
    if floor[i] > 100:
        floor[i] == 100
